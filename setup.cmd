...
set ESP_IDF_TOOLS_PORTABLE_PATH=%DOWNLOADS_DIR%\esp-idf-tools-portable
set IDF_TOOLS_PATH=%ESP_IDF_TOOLS_PORTABLE_PAT%\.espressif

md "%ESP_IDF_TOOLS_PORTABLE_PATH%" &&^
cd /d "%ESP_IDF_TOOLS_PORTABLE_PATH%" &&^
git clone --branch %REF% --recurse-submodules https://github.com/espressif/esp-idf.git &&^
cd /d "%ESP_IDF_TOOLS_PORTABLE_PATH%\esp-idf" &&^
install.bat &&^
export.bat &&^
echo %PATH%
